(() => {
  ...
  // scripts/utils/latex-completions.js
  var LATEX_COMMANDS = [
    // Document structure
    "\\documentclass",
    "\\usepackage",
  ...
  function latexCommandSource(context) {
    ...
    return {
      from: word.from,
      options: LATEX_COMMANDS.map((cmd2) => ({ label: cmd2, type: "keyword" })),
      validFor: /^\\[a-zA-Z]*$/
    };
  }
  ...
  window.initEditor = function({ containerEl, filenameEl, onChange, getCitekeys, onQuickOpen }) {
    ...
    return {
      open(name2, content2) {
        filenameEl.textContent = name2;
        const current = view.state.doc.toString();
        view.dispatch({
          changes: { from: 0, to: current.length, insert: content2 ?? "" },
